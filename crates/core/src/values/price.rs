...
impl Price {
    ...
    /// or `None` without a gross price.
    pub fn net(&self) -> Option<Decimal> {
        Some(self.gross? - self.discount.unwrap_or(Decimal::ZERO))
    }
}
...
mod test {
    ...
    fn nets_the_gross_less_the_discount() {
        assert_eq!(price(1000, Some(150)).net(), Some(Decimal::new(850, 2)));
    }

    #[test]
    fn nets_to_the_gross_without_a_discount() {
        assert_eq!(price(1000, None).net(), Some(Decimal::new(1000, 2)));
    }
}
