...
`Serializable` and `Deserializable` are the core (de)serializers (ADR-0005). They are traits of an invoice type, parameterized by the binding and the namespace set. They serialize a `DocumentBuilder<P>` to XML and parse XML back to one. The writer computes the derived amounts and fills the dictionary in lockstep.
