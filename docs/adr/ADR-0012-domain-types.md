...
A value that selects the terms of a model node keeps a closed type. The VAT category is one such value, since it decides which VAT terms apply. A value that only reaches the XML keeps a looser type, which checks the format alone. A media type and an exemption code are such values.
